...
static LW_OBJECT_HANDLE   _hVNCServerThread;
...
int GUI_VNC_X_StartServer(int LayerIndex, int ServerIndex) {
  LW_CLASS_THREADATTR stThreadAttr;
  ...
  //
  stThreadAttr = API_ThreadAttrGetDefault();

  API_ThreadAttrBuild(&stThreadAttr,
                      8 * 1024,
                      LW_PRIO_LOW,
                      LW_OPTION_THREAD_STK_CHK,
                      LW_NULL);

  _hVNCServerThread = API_ThreadCreate("VNC Server",
                                       (PTHREAD_START_ROUTINE)_ServerTask,
                                       &stThreadAttr,
                                       LW_NULL);
